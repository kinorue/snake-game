...
public class Board extends JPanel
{
  ...
  private void paintScore(Graphics g)
  {
    ...
    int x = (int)Math.round(X_OFFSET*scale);
    int y = (int)Math.round((Y_OFFSET)*scale  ) ;
    
    g.fillRect(x,y, this.getSize().width, (int)Math.round(GAP+FONT_SIZE*scale) );
    g.setColor( Color.red );
    g.drawString(score, x, y + (int)Math.round(FONT_SIZE*scale));
  }
  ...
  public void paintComponent( Graphics g ) 
  {
    ...
    ArrayList<Peg> pegs = Snake.getInstance().getSnakeLocation();

    if (pegs!=null && pegs.size()>1) {
        for (int i = 1; i < pegs.size()-1; i++) {
        	putPeg("blue", pegs.get(i).getRow(),  pegs.get(i).getCol());

        }
        putPeg("green", pegs.get(0).getRow(),  pegs.get(0).getCol());
    }

    
    Peg peg = Snake.getInstance().getPeg();
    if (peg!=null ) {
        putPeg("red", peg.row,peg.col);
    }
  ...
  public void displayScore(String theMessage)
  {
	score = theMessage;
    this.repaint();
  }
